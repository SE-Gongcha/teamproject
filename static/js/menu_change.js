...
add_ok_btn.addEventListener('click', () => {
    if (checkMenu("add")) {
        const new_obj = {
            number: menu_data.length + 1,
            category: category_selectEl.options[category_selectEl.selectedIndex].value,
            name: input_menuEl.value,
            price: input_priceEl.value,
            ice: input_iceEl.checked,
            hot: input_hotEl.checked,
            topping_number: input_toppingEl.value,
            is_waiting: input_waitingEl.checked,
            is_output_kitchen: input_kitchenEl.checked
        };
